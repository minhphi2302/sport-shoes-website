...
<div class="container pb-5">
            ...
            <div class="card-body">
                ...
                <div class="progress mb-3" style="height: 10px;">
                    <div class="progress-bar bg-success" role="progressbar"
                        style="width: <?= $progress ?>%; border-radius: 5px;" aria-valuenow="<?= $progress ?>"
                        aria-valuemin="0" aria-valuemax="100"></div>
                </div>
                <?php if ($progress >= 100): ?>
                    <p class="mb-0 text-muted">Chúc mừng ! Đơn hàng của bạn đã đủ điều kiện được Freeship 🎉 Đến trang thanh
                        toán để nhận giảm giá ngay nào!</p>
                <?php else: ?>
                    <p class="mb-0 text-muted">Mua thêm <span
                            class="fw-bold text-red"><?= number_format($remaining, 0, ',', '.') ?>đ</span> nữa để được miễn phí
                        vận chuyển 🎉</p>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center align-middle">
                                            <div class="quantity-selector mx-auto" style="width: 48px;">
                                                <input type="number" name="quantity[<?= $item['cart_key'] ?>]"
                                                    class="form-control text-center cart-update-btn shadow-none fw-semibold bg-light px-1"
                                                    style="border-radius: 6px; border: 1px solid #e9ecef; height: 30px; font-size: 0.85rem;"
                                                    value="<?= $item['quantity'] ?>" min="1">
                                            </div>
                                        </td>
                                        <td class="text-center align-middle">
                                            <select name="size[<?= $item['cart_key'] ?>]"
                                                class="form-select form-select-sm cart-update-btn shadow-none mx-auto fw-semibold bg-light"
                                                style="width: 44px; border-radius: 6px; border: 1px solid #e9ecef; height: 28px; font-size: 0.8rem; cursor: pointer; padding: 0 12px 0 4px; text-align: center; background-position: right 4px center; background-size: 6px 4px; margin: 0 auto;">
                                        ...
                                        <td class="text-center align-middle">
                                            <select name="color[<?= $item['cart_key'] ?>]"
                                                class="form-select form-select-sm cart-update-btn shadow-none mx-auto fw-semibold bg-light"
                                                style="width: 66px; border-radius: 6px; border: 1px solid #e9ecef; height: 28px; font-size: 0.8rem; cursor: pointer; padding: 0 12px 0 4px; text-align: center; background-position: right 4px center; background-size: 6px 4px; margin: 0 auto;">
                                                <?php foreach ($item['available_colors'] as $c): ?>
                                                    <option value="<?= htmlspecialchars((string) $c) ?>" <?= ($item['color'] ?? 'Black') == $c ? 'selected' : '' ?>><?= htmlspecialchars((string) $c) ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </td>
                                        <td class="text-center align-middle">
                                            <select name="gender[<?= $item['cart_key'] ?>]"
                                                class="form-select form-select-sm cart-update-btn shadow-none mx-auto fw-semibold bg-light"
                                                style="width: 62px; border-radius: 6px; border: 1px solid #e9ecef; height: 28px; font-size: 0.8rem; cursor: pointer; padding: 0 12px 0 4px; text-align: center; background-position: right 4px center; background-size: 6px 4px; margin: 0 auto;">
                                                <?php foreach ($item['available_genders'] as $g): ?>
                                                    <option value="<?= htmlspecialchars($g) ?>" <?= ($item['gender'] ?? 'male') == $g ? 'selected' : '' ?>>
                                                        <?= $g == 'male' ? 'Nam' : ($g == 'female' ? 'Nữ' : ucfirst($g)) ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </td>
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/products" class="btn btn-outline-dark-custom btn-sm"><i
                            class="fa-solid fa-arrow-left me-1"></i> Quay Lại</a>
                    <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/cart/clear" class="btn btn-outline-secondary btn-sm"><i
                            class="fa-solid fa-broom me-1"></i> Xóa tất cả</a>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                <div class="summary-card">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <span
                            class="fw-extrabold fs-4 text-red"><?= number_format($finalAmount ?? $totalAmount, 0, ',', '.') ?>đ</span>
                    </div>
    ...
    <?php else: ?>
        ...
        <div class="text-center py-5">
            ...
            <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/products" class="btn btn-red btn-lg px-4 py-2 mt-2"><i
                    class="fa-solid fa-bag-shopping me-2"></i> KHÁM PHÁ SẢN PHẨM</a>
        </div>
    <?php endif; ?>
</div>
