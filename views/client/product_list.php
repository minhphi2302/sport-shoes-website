            ...
            <div class="filter-sidebar">
                <form action="<?= ($_ENV['APP_URL'] ?? '') ?>/products" method="GET" id="filterForm">
                    ...
                    <div class="filter-group">
                        ...
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="radio" name="category_id" value="" id="cat_all"
                                <?= empty($_GET['category_id']) ? 'checked' : '' ?> onchange="this.form.submit()">
                            ...
                            <?php foreach ($categories as $cat): ?>
                                ...
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="category_id"
                                        value="<?= $cat['category_id'] ?>" id="cat_<?= $cat['category_id'] ?>"
                                        <?= (isset($_GET['category_id']) && $_GET['category_id'] == $cat['category_id']) ? 'checked' : '' ?> onchange="this.form.submit()">
                    ...
                    <div class="filter-group">
                        ...
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="radio" name="brand_id" value="" id="brand_all"
                                <?= empty($_GET['brand_id']) ? 'checked' : '' ?> onchange="this.form.submit()">
                            <label class="form-check-label" for="brand_all">Tất cả thương hiệu</label>
                        </div>
                        <?php if (!empty($brands)): ?>
                            <?php foreach ($brands as $b): ?>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="brand_id" value="<?= $b['brand_id'] ?>"
                                        id="brand_<?= $b['brand_id'] ?>" <?= (isset($_GET['brand_id']) && $_GET['brand_id'] == $b['brand_id']) ? 'checked' : '' ?> onchange="this.form.submit()">
                                    <label class="form-check-label" for="brand_<?= $b['brand_id'] ?>">
                    ...
                    <div class="filter-group">
                        ...
                        <div class="form-check mb-2">
                            <input class="form-check-input" type="radio" name="gender" value="" id="gender_all"
                                <?= empty($_GET['gender']) ? 'checked' : '' ?> onchange="this.form.submit()">
                    ...
                    <!-- Chọn Size Giày -->
                    <div class="filter-group">
                        <h6 class="filter-title"><i class="fa-solid fa-shoe-prints me-2 text-red"></i> Kích thước (Size)
                        </h6>
                        <div class="size-btn-group flex-wrap gap-2 d-flex">
                            ...
                            foreach ($sizes as $s):
                                ?>
                                <input type="radio" class="btn-check" name="size" id="size_<?= $s ?>" value="<?= $s ?>"
                                    autocomplete="off" <?= (isset($_GET['size']) && $_GET['size'] == $s) ? 'checked' : '' ?>
                                    onchange="this.form.submit()">
                                <label class="btn btn-outline-dark" for="size_<?= $s ?>">
                                    <?= $s ?>
                                </label>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            foreach ($colors as $c):
                                ...
                                <input type="radio" class="btn-check" name="color" id="<?= $colorId ?>"
                                    value="<?= htmlspecialchars((string) $c) ?>" <?= $isChecked ?> autocomplete="off"
                                    onchange="this.form.submit()">
                    ...
                    <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/products"
                        class="btn btn-outline-secondary w-100 mt-2 btn-sm">Xóa tất cả bộ lọc</a>
                </form>
            </div>
        </div>
