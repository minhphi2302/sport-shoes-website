    ...
    <div class="row">
        <div class="col-md-3 mb-4">
            <div class="list-group rounded-4 shadow-sm">
                <a href="<?= htmlspecialchars($_ENV['APP_URL'] ?? '') ?>/profile" class="list-group-item list-group-item-action py-3">
                    <i class="bi bi-person me-2"></i> Tài khoản của tôi
                </a>
                <a href="<?= htmlspecialchars($_ENV['APP_URL'] ?? '') ?>/orders" class="list-group-item list-group-item-action active py-3">
        ...
        <div class="col-md-9">
            ...
            <?php if (empty($orders)): ?>
                <div class="alert alert-info text-center py-5 rounded-4 border-0 shadow-sm">
                    ...
                    <a href="<?= htmlspecialchars($_ENV['APP_URL'] ?? '') ?>/products" class="btn btn-primary rounded-pill px-4 mt-2">Bắt đầu mua sắm</a>
                </div>
            <?php else: ?>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="ps-4 fw-semibold text-primary">#<?= str_pad($order['order_id'], 6, '0', STR_PAD_LEFT) ?></td>
                                        ...
                                        <td class="pe-4 text-end">
                                            <a href="<?= htmlspecialchars($_ENV['APP_URL'] ?? '') ?>/orders/<?= $order['order_id'] ?>" class="btn btn-outline-primary btn-sm rounded-pill px-3">Xem</a>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
