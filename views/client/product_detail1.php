    ...
    <div class="container">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item"><a href="<?= ($_ENV['APP_URL'] ?? '') ?>/products"
                        class="text-decoration-none text-dark">Sản phẩm</a></li>
                <li class="breadcrumb-item active text-red fw-semibold" aria-current="page">
...
<div class="container pb-5">
    <div class="row g-5">
        ...
        <div class="col-lg-6">
            <div class="ps-lg-3">
                <span class="badge bg-dark text-white px-3 py-2 text-uppercase mb-2">Thương hiệu:
                    <?= htmlspecialchars($product['brand_name'] ?? 'Chính Hãng') ?>
                </span>
                <h1 class="product-detail-title mb-3">
                    <?= htmlspecialchars($product['name'] ?? 'Nike Air Zoom Pegasus 39') ?>
                </h1>
                ...
                <div class="d-flex align-items-center mb-3">
                    ...
                    <div
                        class="border-start ps-3 <?= ($product['quantity'] ?? 50) > 0 ? 'text-success' : 'text-danger' ?> fw-semibold">
                        <?php if (($product['quantity'] ?? 50) > 0): ?>
                            <i class="fa-solid fa-check-circle me-1"></i> Còn hàng (
                            <?= htmlspecialchars($product['quantity'] ?? 50) ?> sản phẩm)
                        <?php else: ?>
                ...
                <div class="bg-light p-3 rounded-3 mb-4 d-flex align-items-baseline gap-3">
                    <?php if (!empty($product['sale_price']) && $product['sale_price'] < $product['price']): ?>
                        <span class="fs-2 fw-bold text-red">
                            <?= number_format($product['sale_price'], 0, ',', '.') ?>đ
                        </span>
                        <span class="fs-5 text-muted text-decoration-line-through">
                            <?= number_format($product['price'], 0, ',', '.') ?>đ
                        </span>
                        <span class="badge bg-red fs-6 align-middle" style="transform: translateY(-2px);">Tiết kiệm
                            <?= number_format($product['price'] - $product['sale_price'], 0, ',', '.') ?>đ
                        </span>
                    <?php else: ?>
                        <span class="fs-2 fw-bold text-red">
                            <?= number_format($product['price'] ?? 3000000, 0, ',', '.') ?>đ
                        </span>
                    <?php endif; ?>
                </div>
                ...
                <form id="addToCartForm" action="<?= ($_ENV['APP_URL'] ?? '') ?>/cart/add" method="POST">
                        ...
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            <?php foreach (array_values($availableColors) as $idx => $c): ?>
                                ...
                                <input type="radio" class="btn-check" name="color" id="<?= $colorId ?>"
                                    value="<?= htmlspecialchars((string) $c) ?>" <?= $idx === 0 ? 'checked' : '' ?>>
                                <label class="btn btn-outline-dark btn-sm rounded-pill" for="<?= $colorId ?>">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="quantity-selector input-group" style="width: 150px;">
                            <button class="btn btn-outline-secondary" type="button" onclick="decrementQuantity()"
                                <?= (($product['quantity'] ?? 50) <= 0) ? 'disabled' : '' ?>><i
                                    class="fa-solid fa-minus"></i></button>
                            <input type="number" id="quantityInput" name="quantity" class="form-control text-center"
                                value="1" min="1" max="<?= $product['quantity'] ?? 50 ?>" <?= (($product['quantity'] ?? 50) <= 0) ? 'disabled' : '' ?>>
                    ...
                    <div class="d-grid gap-2 d-md-flex mt-4">
                        <button type="submit" name="action" value="add"
                            class="btn btn-outline-dark-custom btn-lg flex-fill py-3" <?= (($product['quantity'] ?? 50) <= 0) ? 'disabled' : '' ?>>
                            <i class="fa-solid fa-cart-plus me-2"></i> THÊM VÀO GIỎ
                        </button>
                        <button type="submit" name="action" value="buy_now" class="btn btn-red btn-lg flex-fill py-3"
                            <?= (($product['quantity'] ?? 50) <= 0) ? 'disabled' : '' ?>>
    ...
    <div class="mt-5 pt-4">
        <ul class="nav nav-tabs nav-tabs-custom mb-4" id="productTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active fw-bold text-uppercase" id="desc-tab" data-bs-toggle="tab"
                    data-bs-target="#desc-tab-pane" type="button" role="tab">Mô tả chi tiết</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link fw-bold text-uppercase" id="specs-tab" data-bs-toggle="tab"
                    data-bs-target="#specs-tab-pane" type="button" role="tab">Thông số kỹ thuật</button>
            </li>
        </ul>
                ...
                <table class="table table-striped table-bordered mb-0">
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <?= htmlspecialchars($product['brand_name'] ?? 'Đang cập nhật') ?>
                            </td>
                        </tr>
                        <tr>
                            <th>Mã sản phẩm</th>
                            <td>
                                <?= htmlspecialchars($product['sku'] ?? 'Đang cập nhật') ?>
                            </td>
                        </tr>
                        <tr>
                            <th>Giới tính</th>
                            <td>
                                <?= htmlspecialchars($product['gender'] === 'male' ? 'Nam' : ($product['gender'] === 'female' ? 'Nữ' : 'Unisex')) ?>
                            </td>
                        </tr>
                        <tr>
                            <th>Trạng thái</th>
                            <td>
                                <?= htmlspecialchars($product['status'] === 'active' ? 'Đang bán' : 'Ngừng kinh doanh') ?>
                            </td>
                        </tr>
                    </tbody>
                </table>
                            ...
                            <div class="product-body">
                                ...
                                <?php if (!empty($rel['sku'])): ?>
                                    <div class="text-muted mb-2" style="font-size: 0.85rem;">
                                        <?= htmlspecialchars($rel['sku']) ?>
                                    </div>
                                <?php endif; ?>
                                <div class="product-price-box">
                                    <?php if (($rel['sale_price'] ?? 0) > 0 && $rel['sale_price'] < $rel['price']): ?>
                                        <span class="product-price text-danger">
                                            <?= number_format($rel['sale_price'], 0, ',', '.') ?>đ
                                        </span>
                                        <span class="product-price-old text-decoration-line-through text-muted ms-2"
