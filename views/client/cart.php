...
<div class="container py-5">
    ...
    <?php if (empty($cart)): ?>
        <div class="text-center py-5 bg-white rounded-4 shadow-sm border-0">
            ...
            <a href="<?= base_url('products') ?>" class="btn btn-primary rounded-pill px-5 py-3 fw-bold"><i
                    class="bi bi-shop me-2"></i> Khám phá sản phẩm ngay</a>
        </div>
    <?php else: ?>
        <div class="row g-4">
            <!-- Danh sách sản phẩm -->
                        ...
                        <div class="table-responsive">
                            <table class="table align-middle mb-0">
                                <thead class="table-light">
                                    <tr>
                                        <th scope="col" class="border-0 text-uppercase fw-bold text-muted ps-3">Sản phẩm
                                        </th>
                                        <th scope="col" class="border-0 text-uppercase fw-bold text-muted text-center">Số
                                            lượng</th>
                                        <th scope="col" class="border-0 text-uppercase fw-bold text-muted text-end">Thành
                                            tiền</th>
                                        <th scope="col" class="border-0 text-uppercase fw-bold text-muted text-end pe-3">
                                            Thao tác</th>
                                    </tr>
                                </thead>
                                <tbody>
                                                ...
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div>
                                                        ...
                                                        <?php if (!empty($item['sku'])): ?>
                                                            <small class="text-muted d-block" style="font-size: 0.8rem;">Mã SKU:
                                                                <?= htmlspecialchars($item['sku']) ?>
                                                            </small>
                                                        <?php endif; ?>
                                                        <?php if (!empty($item['size'])): ?>
                                                            <small class="text-muted d-block" style="font-size: 0.8rem;">Size: <span
                                                                    class="fw-semibold text-dark">
                                                                    <?= htmlspecialchars((string) $item['size']) ?>
                                                                </span></small>
                                                        <?php endif; ?>
                                                        <?php if (!empty($item['color'])): ?>
                                                            <small class="text-muted d-block" style="font-size: 0.8rem;">Màu sắc:
                                                                <span class="fw-semibold text-dark">
                                                                    <?= htmlspecialchars((string) $item['color']) ?>
                                                                </span></small>
                                                        <?php endif; ?>
                                            ...
                                            <td class="text-center">
                                                <form action="<?= base_url('cart/update') ?>" method="POST"
                                                    ...
                                                    <input type="hidden" name="cart_key"
                                                        value="<?= $item['cart_key'] ?? $item['product_id'] ?>">
                                                    <input type="number" name="quantity" value="<?= $item['quantity'] ?>"
                                                        min="1" class="form-control text-center rounded-3 mx-1"
                    ...
                    <div class="card-body p-4">
                        ...
                        <?php if ($needed > 0): ?>
                            <div class="alert alert-warning border-0 rounded-3 mb-3 p-3" style="font-size: 0.85rem;">
                                <i class="bi bi-truck me-1"></i> Mua thêm <strong>
                                    <?= number_format($needed, 0, ',', '.') ?>đ
                                </strong> để được <strong class="text-success">Miễn phí vận chuyển (Freeship)</strong>!
                            </div>
                        <?php else: ?>
                            <div class="alert alert-success border-0 rounded-3 mb-3 p-3" style="font-size: 0.85rem;">
                                <i class="bi bi-check-circle-fill me-1"></i> Đơn hàng của bạn đã đạt mốc <strong
                                    class="text-success">Miễn phí vận chuyển (Freeship)</strong>!
                            </div>
                        <?php endif; ?>
                        ...
                        <a href="<?= base_url('checkout') ?>"
                            class="btn btn-red w-100 py-3 rounded-pill fw-bold text-uppercase fs-5">
