...
<div class="container pb-5">
    <div class="row g-4">
        ...
        <div class="col-lg-3">
            ...
            <p class="mb-4">Xin chào, <span class="fw-bold">
                    <?= htmlspecialchars($name) ?> !
                </span></p>

            <ul class="nav flex-column list-unstyled mb-4" id="accountTabs" role="tablist">
                <li class="mb-3 nav-item" role="presentation">
                    <a href="#info" class="nav-link text-dark fw-bold px-0 active" id="info-tab" data-bs-toggle="tab"
                        data-bs-target="#info-pane" type="button" role="tab" aria-controls="info-pane"
                        aria-selected="true" onclick="changeActiveTab(this)">Thông tin tài khoản</a>
                </li>
                <li class="mb-3 nav-item" role="presentation">
                    <a href="#address" class="nav-link text-dark px-0" id="address-tab" data-bs-toggle="tab"
                        data-bs-target="#address-pane" type="button" role="tab" aria-controls="address-pane"
                        aria-selected="false" onclick="changeActiveTab(this)">Địa chỉ (1)</a>
                </li>
                <li class="nav-item">
        ...
        <div class="col-lg-9 tab-content" id="accountTabsContent">
            ...
            <div class="tab-pane fade show active" id="info-pane" role="tabpanel" aria-labelledby="info-tab"
                ...
                <p class="mb-3">Tên tài khoản: <span class="fw-bold" id="infoAddrName">
                        <?= htmlspecialchars($name) ?>
                    </span>!</p>
                <p class="mb-3"><i class="fa-solid fa-house text-dark me-2"></i> Địa chỉ: <span id="infoAddrDetail">
                        <?= htmlspecialchars($address) ?>
                    </span></p>
                <p class="mb-4"><i class="fa-solid fa-mobile-screen-button text-dark me-2"></i> Điện thoại: <span
                        id="infoAddrPhone">
                    ...
                    <table class="table border-bottom">
                        <thead>
                            <tr class="align-middle">
                                <th scope="col" class="border-0 bg-transparent text-dark py-3 text-center">Mã đơn hàng
                                </th>
                                <th scope="col" class="border-0 bg-transparent text-dark py-3 text-center">Ngày đặt</th>
                                <th scope="col" class="border-0 bg-transparent text-dark py-3 text-center">Thành tiền
                                </th>
                                <th scope="col" class="border-0 bg-transparent text-dark py-3 text-center">Phương thức
                                    thanh toán</th>
                                <th scope="col" class="border-0 bg-transparent text-dark py-3 text-center">Trạng Thái
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (!empty($orders)): ?>
                                <?php foreach ($orders as $order): ?>
                                    <tr class="align-middle">
                                        <td class="fw-bold text-dark py-3 text-center">#
                                            <?= str_pad($order['order_id'], 5, '0', STR_PAD_LEFT) ?>
                                        </td>
                                        <td class="text-secondary py-3 text-center">
                                            <?= date('d/m/Y H:i', strtotime($order['created_at'])) ?>
                                        </td>
                                        <td class="fw-bold text-red py-3 text-center">
            ...
            <div class="tab-pane fade" id="address-pane" role="tabpanel" aria-labelledby="address-tab" tabindex="0">
                ...
                <button type="button" class="btn btn-dark rounded-1 px-4 py-2 mb-4" data-bs-toggle="modal"
                    data-bs-target="#addressModal" onclick="prepareAddAddress()">Thêm địa chỉ</button>
                ...
                <div class="d-flex justify-content-between align-items-start" id="addressDisplayBlock">
                    <div>
                        <p class="mb-2 text-dark">
                            Họ tên: <span id="displayAddrName">
                                <?= htmlspecialchars($name) ?>
                            </span>
                            <span class="text-success ms-2" id="displayAddrDefault" style="font-size: 0.85rem;"><i
                                    class="fa-regular fa-circle-check"></i> Địa chỉ mặc định</span>
                        </p>
                        <p class="mb-0 text-dark mt-1">Điện thoại: <span id="displayAddrPhone">
                                <?= htmlspecialchars($phone) ?>
                            </span></p>
                        <p class="mb-0 text-dark mt-1">Địa chỉ: <span id="displayAddrDetail">
                                <?= htmlspecialchars($address) ?>
                            </span></p>
                    </div>
                    <!-- Nút Chỉnh sửa gọi Modal -->
                    <a href="javascript:void(0)" class="text-decoration-none text-info" data-bs-toggle="modal"
                        data-bs-target="#addressModal" onclick="prepareEditAddress()">Chỉnh sửa địa chỉ</a>
                </div>
            </div>
            ...
            <div class="modal-body">
                <form id="addressForm">
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <button type="button" class="btn btn-light border rounded-1 px-4"
                            data-bs-dismiss="modal">Hủy</button>
                        <button type="button" class="btn btn-dark rounded-1 px-4" data-bs-dismiss="modal"
                            onclick="saveAddress()">Lưu địa chỉ</button>
                    </div>
                </form>
            </div>
        </div>
