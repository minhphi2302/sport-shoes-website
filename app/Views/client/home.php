...
<div id="heroCarousel" class="carousel slide hero-slider" data-bs-ride="carousel" data-bs-interval="4000">
    ...
    <div class="carousel-inner">
        <div class="carousel-item active">
            <img src="<?= base_url('image/slide/slide1.avif') ?>" class="d-block w-100 hero-slider-img" alt="Slide 1" onerror="this.src='<?= base_url('image/logo.webp') ?>'">
            <div class="carousel-caption carousel-caption-custom d-none d-md-block">
                <h1 class="text-white">PHONG CÁCH THỂ THAO<br><span class="text-red">ĐỈNH CAO</span></h1>
                <p>Khám phá bộ sưu tập giày thể thao chính hãng mới nhất</p>
                <a href="<?= base_url('products') ?>" class="btn btn-red btn-lg px-5 py-3 fw-bold text-uppercase">MUA NGAY</a>
            </div>
        </div>
        <div class="carousel-item">
            <img src="<?= base_url('image/slide/slide2.avif') ?>" class="d-block w-100 hero-slider-img" alt="Slide 2" onerror="this.src='<?= base_url('image/logo.webp') ?>'">
            <div class="carousel-caption carousel-caption-custom d-none d-md-block">
                <h1 class="text-white">GIÀY CHẠY BỘ<br><span class="text-red">HIỆU NĂNG CAO</span></h1>
                <p>Công nghệ đệm tiên tiến — thoải mái từng bước chân</p>
                <a href="<?= base_url('products?sort=newest') ?>" class="btn btn-red btn-lg px-5 py-3 fw-bold text-uppercase">KHÁM PHÁ</a>
            </div>
        </div>
        <div class="carousel-item">
            <img src="<?= base_url('image/slide/slide3.avif') ?>" class="d-block w-100 hero-slider-img" alt="Slide 3" onerror="this.src='<?= base_url('image/logo.webp') ?>'">
            <div class="carousel-caption carousel-caption-custom d-none d-md-block">
                <h1 class="text-white">SIÊU KHUYẾN MÃI<br><span class="text-red">ĐẾN 30%</span></h1>
                <p>Ưu đãi đặc biệt — Số lượng có hạn!</p>
                <a href="<?= base_url('products?sale=1') ?>" class="btn btn-red btn-lg px-5 py-3 fw-bold text-uppercase">XEM NGAY</a>
            </div>
        </div>
    </div>
                    ...
                    <?php foreach ($brands as $brand): ?>
                        <div class="col-6 col-md-4 col-lg-2">
                            <a href="<?= base_url('products?brand_id=' . $brand['brand_id']) ?>" class="brand-card">
                                ...
                                ?>
                                <img src="<?= htmlspecialchars($logo) ?>" alt="<?= htmlspecialchars($brand['name']) ?>"
                                    class="brand-logo-img" onerror="this.src='<?= base_url('image/logo.webp') ?>'">
    ...
    <!-- Featured Products (Mới Nhất) -->
    <section class="container mb-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="fw-bold m-0">Sản Phẩm Mới Nhất</h2>
            <a href="<?= htmlspecialchars($_ENV['APP_URL'] ?? '') ?>/products?sort=newest" class="text-decoration-none fw-bold">Xem
                tất cả <i class="bi bi-arrow-right"></i></a>
        </div>
        <div class="row g-4">
                ...
                <?php foreach (array_slice($featuredProducts, 0, 4) as $product): ?>
                    <div class="col-6 col-md-4 col-lg-3">
                        <div class="product-card h-100 border rounded-2 bg-white overflow-hidden position-relative">
                            ...
                            <?php $imgSrc = get_product_image_url($product['image_url'] ?? null, $product['product_id'] ?? 1); ?>
    ...
    <section class="py-5" id="sieu-khuyen-mai">
        <div class="container position-relative">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-4">
                <h2 class="section-title mb-3 mb-lg-0 text-uppercase fw-bold" style="font-size: 1.5rem;"><i
                        class="fa-solid fa-bolt me-2 text-warning"></i>SIÊU KHUYẾN MÃI</h2>
                <div class="d-flex flex-wrap gap-2">
                    <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/products?sale=1"
                        class="btn btn-brand-filter bg-transparent border btn-sm px-3 py-2 text-dark fw-semibold text-uppercase"
                        style="font-size: 0.85rem;">XEM TẤT CẢ</a>
                </div>
            </div>
                            ...
                            <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
                                <div class="row g-3">
                                    <?php foreach ($chunk as $product): ?>
                                        <div class="col-6 col-md-4 col-lg-3">
                                            <div
                                                class="card h-100 rounded-1 border promo-card overflow-hidden bg-white product-card">
                                                <?php
                                                $discountPercent = round((($product['price'] - $product['sale_price']) / $product['price']) * 100);
                                                ?>
                                                <div class="badge-ribbon badge-ribbon-red"><small>-</small><?= $discountPercent ?>%
                                                </div>
                                                <?php
                                                ...
                                                <div class="position-relative">
                                                    <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/product/<?= $product['product_id'] ?>"
                                                        ...
                                                        <div style="background-color: #f8f9fa;" class="text-center p-2">
                                                            <img src="<?= htmlspecialchars($imgSrc) ?>" class="img-fluid"
                                                                style="height: 190px; object-fit: contain; width: 100%;"
                                                                alt="<?= htmlspecialchars($product['name']) ?>"
                                                                onerror="this.src='<?= base_url('image/slide/slide1.avif') ?>'">
                                                        </div>
                                                    </a>
                                                    <div class="product-center-action">
                                                        <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/product/<?= $product['product_id'] ?>"
                                                    ...
                                                    <?php if (($product['quantity'] ?? 50) > 0): ?>
                                                        <a href="<?= base_url('cart/add/' . $product['product_id']) ?>"
                                                            class="btn-cart-hover position-absolute" style="bottom: 10px; right: 10px;"
                                                            title="Thêm vào giỏ">
                                                            <span class="cart-text">Thêm vào giỏ</span>
                                                            <span class="cart-icon-wrapper">
                                                                <svg class="custom-bag-icon" viewBox="0 0 24 24" fill="none"
                                                                    ...
                                                                    <circle cx="18" cy="18" r="5" class="bag-bg-circle" stroke="none">
                                                                    </circle>
                                                    ...
                                                    <?php else: ?>
                                                        <a href="<?= base_url('cart/add/' . $product['product_id']) ?>"
                                                            class="btn-cart-hover position-absolute btn-out-of-stock"
                                                            style="bottom: 10px; right: 10px;" title="Tạm hết hàng">
                                                            <span class="cart-text">Tạm hết hàng</span>
                                                            <span class="cart-icon-wrapper">
                                                                <svg class="custom-bag-icon" viewBox="0 0 24 24" fill="none"
                                                                    stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
                                                ...
                                                <div class="card-body px-2 py-3 text-start">
                                                    ...
                                                    <div class="d-flex justify-content-start align-items-baseline gap-2 flex-wrap">
                                                        <span class="text-danger fw-bold"
                                                            style="font-size: 1.1rem;"><?= number_format($product['sale_price'], 0, ',', '.') ?>đ</span>
                                                        <span class="text-muted text-decoration-line-through"
                                                            style="font-size: 0.85rem;"><?= number_format($product['price'], 0, ',', '.') ?>đ</span>
                                                        <?php if (isset($product['quantity']) && $product['quantity'] <= 0): ?>
                                                            <span class="badge bg-danger rounded-1 ms-2 p-1 px-2 fw-medium"
                                                                style="font-size: 0.75rem;">Hết hàng</span>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                            </div>
    ...
    <section class="py-5">
        <div class="container">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-4">
                <h2 class="section-title mb-3 mb-lg-0 text-uppercase fw-bold" style="font-size: 1.5rem;">GIÀY SNEAKER
                </h2>
                <div class="d-flex flex-wrap gap-2" id="home-brand-filters">
                    <?php if (!empty($brands)): ?>
                        <?php foreach ($brands as $b): ?>
                            <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/products?brand_id=<?= $b['brand_id'] ?>"
                                class="btn btn-brand-filter bg-transparent border btn-sm px-3 py-2 text-dark fw-semibold text-uppercase"
                                style="font-size: 0.85rem;" data-brand-id="<?= $b['brand_id'] ?>">GIÀY
                                <?= htmlspecialchars($b['name']) ?></a>
                        <?php endforeach; ?>
                    <?php endif; ?>
                    <a href="#"
                        class="btn btn-brand-filter bg-transparent border btn-sm px-3 py-2 text-dark fw-semibold text-uppercase active"
                        style="font-size: 0.85rem;" data-brand-id="">TẤT CẢ</a>
                </div>
            </div>
                                ...
                                <div class="product-card">
                                    <div class="product-img-wrapper">
                                        ...
                                        <?php if ($hasSale): ?>
                                            ...
                                            $discountPercent = round((($product['price'] - $product['sale_price']) / $product['price']) * 100);
                                            ?>
                                            <div class="badge-ribbon badge-ribbon-red" <?= $isNew ? 'style="left: 43px;"' : '' ?>>
                                                <small>-</small><?= $discountPercent ?>%
                                            </div>
                                        <?php endif; ?>
                                        ...
                                        <?php if (($product['quantity'] ?? 50) > 0): ?>
                                            <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/cart/add/<?= $product['product_id'] ?>"
                                                class="btn-cart-hover" title="Thêm vào giỏ">
                                                <span class="cart-text">Thêm vào giỏ</span>
                                                <span class="cart-icon-wrapper">
                                                    <svg class="custom-bag-icon" viewBox="0 0 24 24" fill="none"
                                                        stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
                                        ...
                                        <?php else: ?>
                                            <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/cart/add/<?= $product['product_id'] ?>"
                                                class="btn-cart-hover btn-out-of-stock" title="Tạm hết hàng">
                                                <span class="cart-text">Tạm hết hàng</span>
                                                <span class="cart-icon-wrapper">
                                                    <svg class="custom-bag-icon" viewBox="0 0 24 24" fill="none"
                                                        stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
                                    ...
                                    <div class="product-body">
                                        ...
                                        <div class="product-price-box">
                                            <?php if (($product['sale_price'] ?? 0) > 0 && $product['sale_price'] < $product['price']): ?>
                                                <span
                                                    class="product-price"><?= number_format($product['sale_price'], 0, ',', '.') ?>đ</span>
                                                <span
                                                    class="product-price-old"><?= number_format($product['price'], 0, ',', '.') ?>đ</span>
                                            <?php else: ?>
                                                <span
                                ...
                                <div class="product-card">
                                    <div class="product-img-wrapper">
                                        ...
                                        <?php if (($dp['quantity'] ?? 50) > 0): ?>
                                            <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/cart/add/<?= $dp['id'] ?>"
                                                class="btn-cart-hover" title="Thêm vào giỏ">
                                                <span class="cart-text">Thêm vào giỏ</span>
                                                <span class="cart-icon-wrapper">
                                                    <svg class="custom-bag-icon" viewBox="0 0 24 24" fill="none"
                                    ...
                                    <div class="product-body">
                                        ...
                                        <div class="product-price-box">
                                            <?php if ($dp['sale']): ?>
                                                ...
                                                <span
                                                    class="product-price-old"><?= number_format($dp['price'], 0, ',', '.') ?>đ</span>
                                            <?php else: ?>
    ...
    <section class="py-5 bg-dark text-white my-4 position-relative overflow-hidden" style="--promo-color: #ff006a;">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-lg-6">
                    <span class="badge fs-6 mb-2 text-white border-0" style="background-color: var(--promo-color);">ƯU
                        ĐÃI KHỦNG THÁNG NÀY</span>
                    <h2 class="display-5 fw-bold text-uppercase text-white mb-3">SĂN GIÀY HIỆU - SIÊU SALE NỬA GIÁ</h2>
                    <p class="fs-5 text-secondary mb-4">
                        Nhập mã <strong class="border px-2 py-1 rounded"
                            style="border-color: var(--promo-color) !important; color: var(--promo-color) !important;">ANTA2026</strong>
                        giảm ngay 20% cho đơn hàng đầu tiên từ 1.500.000đ.
                    </p>
                    <a href="<?= ($_ENV['APP_URL'] ?? '') ?>/products?sale=1"
                        class="btn btn-lg px-4 py-3 text-white border-0 fw-bold shadow-sm"
                        style="background-color: var(--promo-color); transition: all 0.3s;"
                        onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'"><i
                            class="fa-solid fa-bolt me-2"></i> MUA NGAY KẺO LỠ</a>
                </div>
                <div class="col-lg-6 d-none d-lg-block text-center">
                    <img src="<?= base_url('image/khuyen_mai.png') ?>" alt="Promo Shoe"
                        class="img-fluid rounded-4 shadow-lg border border-2"
                        style="max-height: 350px; object-fit: cover; border-color: var(--promo-color) !important;">
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="row justify-content-center">
                <div class="col-lg-10 col-xl-9 text-center">
                    ...
                    <p class="mb-4 text-secondary" style="line-height: 1.8; font-size: 1.05rem; text-align: justify;">
                        Khám phá những đôi giày thể thao hiệu năng cao, ứng dụng công nghệ tiên tiến nhất để bạn chinh
                        phục mọi giới hạn. Dù bạn là người đam mê chạy bộ, yêu thích bóng rổ hay đơn giản là theo đuổi
                        lối sống năng động, ANTA Store luôn là điểm đến lý tưởng. Chúng tôi đồng hành để bạn tập luyện
                        bền bỉ hơn, di chuyển linh hoạt hơn với những sản phẩm luôn theo kịp từng bước chân của bạn.
                    </p>

                    <p class="mb-5 text-secondary" style="line-height: 1.8; font-size: 1.05rem; text-align: justify;">
                        Cửa hàng giày thể thao ANTA không chỉ là nơi mua sắm, mà còn là không gian để bạn nâng tầm phong
                        cách và bứt phá giới hạn bản thân. Mỗi đôi giày đều được chế tác tỉ mỉ, chú trọng tuyệt đối vào
                        độ êm ái, độ bền và chi tiết thiết kế. Dù bạn chơi môn thể thao nào, ANTA luôn sẵn sàng đồng
                        hành – giúp bạn tự tin tỏa sáng và từng bước vươn tới đỉnh cao.
                    </p>

                    <div class="mt-4">
                        <img src="<?= base_url('image/logo.webp') ?>" alt="ANTA Logo"
